        ...
        <div style="display:flex;flex-direction:column;gap:12px">
            @forelse($earlyBirds as $index => $bird)
                ...
                <div class="bird-item" data-schedule='@json($bird['schedule'])' style="display:flex;align-items:center;gap:12px;padding:{{ $padding }};border-bottom:{{ $rank < 5 ? '1px solid #f1f5f9' : 'none' }}">
                    ...
                    @if($bird['photo'])
                        <img src="{{ $bird['photo'] }}" style="width:{{ $avatarSize }}px;height:{{ $avatarSize }}px;border-radius:10px;object-fit:cover;border:2px solid var(--theme-neutral-300);flex-shrink:0">
                    @else
                        <div class="emp-avatar-dynamic" data-bg="{{ $bird['color'] }}" style="width:{{ $avatarSize }}px;height:{{ $avatarSize }}px;border-radius:10px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:{{ $rank === 1 ? '18px' : ($rank === 2 ? '16px' : '14px') }};border:2px solid var(--theme-neutral-300);flex-shrink:0">{{ $bird['initials'] }}</div>
                    @endif
                    <div style="flex:1;min-width:0">
                        <p style="font-size:{{ $nameSize }};font-weight:{{ $rank === 1 ? '700' : '600' }};color:var(--theme-neutral-900);margin:0 0 2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $bird['name'] }}</p>
                        <p style="font-size:{{ $positionSize }};color:var(--theme-neutral-700);margin:0;font-weight:500">{{ $bird['position'] }}</p>
                    </div>
                    <div style="text-align:right;flex-shrink:0;margin-right:16px">
                        <p style="font-size:{{ $timeSize }};font-weight:700;color:var(--gp-pri);margin:0">{{ $bird['time_in'] }}</p>
                    </div>
                </div>
            @empty
                <div style="text-align:center;padding:32px 24px">
                    <div style="width:48px;height:48px;border-radius:50%;background:var(--theme-neutral-100);display:flex;align-items:center;justify-content:center;margin:0 auto 12px">
                    ...
                    <p style="font-size:13px;font-weight:600;color:var(--theme-neutral-700);margin:0 0 4px">No Attendance Records Today</p>
                    <p style="font-size:12px;color:var(--theme-neutral-600);margin:0">Check back later as employees clock in</p>
                </div>
            @endforelse
        </div>
    </div>
        ...
        <div style="display:flex;flex-direction:column;gap:12px">
            @forelse($lateBirds as $index => $bird)
                ...
                <div class="bird-item" data-schedule='@json($bird['schedule'])' style="display:flex;align-items:center;gap:12px;padding:{{ $padding }};border-bottom:{{ $rank < 5 ? '1px solid #fee2e2' : 'none' }}">
                    ...
                    <div style="text-align:right;flex-shrink:0;margin-right:16px">
                        <p style="font-size:{{ $timeSize }};font-weight:700;color:var(--gp-pri);margin:0 0 3px">{{ $bird['time_in'] }}</p>
                        <p style="font-size:10px;color:var(--theme-danger);font-weight:600;margin:0">+{{ $bird['late_minutes'] }}m</p>
                    </div>
                </div>
            @empty
                <div style="text-align:center;padding:32px 24px">
                    ...
                    <p style="font-size:13px;font-weight:600;color:var(--theme-neutral-700);margin:0 0 4px">No Late Arrivals Today</p>
                    <p style="font-size:12px;color:var(--theme-neutral-600);margin:0">All employees arrived on time! 🎉</p>
                </div>
            @endforelse
        </div>
    </div>
...
@push('scripts')
    @vite('resources/js/admin/dashboard/early-birds-card.js')
@endpush
