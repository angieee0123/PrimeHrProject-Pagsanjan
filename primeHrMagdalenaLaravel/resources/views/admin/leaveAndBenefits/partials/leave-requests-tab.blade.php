<section class="table-section" id="leave-tab" style="display: block; border: 1px solid var(--theme-neutral-300); border-radius: 12px; background: #fff; box-shadow: 0 2px 8px rgba(15, 23, 42, .04), 0 1px 3px rgba(15, 23, 42, .03); overflow: hidden;">
    <div class="table-header" style="background: linear-gradient(135deg, var(--gp-bg-tint-2) 0%, #fff 100%); padding: 18px 20px; border-bottom: 1px solid var(--theme-neutral-300); align-items: center;">
        <div>
            <h3 class="table-title" style="color: var(--theme-neutral-950); font-size: 15px; font-weight: 800; margin: 0 0 4px;">Leave Requests — {{ now()->format('F Y') }}</h3>
            <p class="table-sub" style="color: var(--theme-neutral-700); font-size: 12px; margin: 0;">Municipal Government of Pagsanjan · <span id="leaveRequestCount">{{ $leaveApplications->count() }}</span> records</p>
        </div>
        <div class="table-actions" style="display: flex; gap: 8px; flex-wrap: wrap;">
            ...
            <button class="btn-export" style="background: var(--theme-danger); color: #fff; border: 1px solid var(--theme-danger); border-radius: 8px; font-size: 11px; padding: 0 14px; height: 34px; font-weight: 700; display: flex; align-items: center; gap: 6px; transition: all 0.2s; box-shadow: 0 2px 4px rgba(220, 38, 38, 0.1);" onclick="openManualCreditModal('deduct')" onmouseover="this.style.background='#b91c1c'; this.style.boxShadow='0 4px 8px rgba(220, 38, 38, 0.2)'" onmouseout="this.style.background='#dc2626'; this.style.boxShadow='0 2px 4px rgba(220, 38, 38, 0.1)'">
        ...
        <table class="payroll-table" style="width: 100%; border-collapse: separate; border-spacing: 0;">
            <thead>
                <tr>
                    ...
                    <th style="position: sticky; top: 0; z-index: 2; background: var(--theme-neutral-50); color: var(--theme-neutral-700); font-size: 10.5px; font-weight: 800; text-transform: uppercase; padding: 12px 16px; text-align: left; border-bottom: 1px solid var(--theme-neutral-200);">Leave Type</th>
                    <th style="position: sticky; top: 0; z-index: 2; background: var(--theme-neutral-50); color: var(--theme-neutral-700); font-size: 10.5px; font-weight: 800; text-transform: uppercase; padding: 12px 16px; text-align: left; border-bottom: 1px solid var(--theme-neutral-200);">Period</th>
                    <th style="position: sticky; top: 0; z-index: 2; background: var(--theme-neutral-50); color: var(--theme-neutral-700); font-size: 10.5px; font-weight: 800; text-transform: uppercase; padding: 12px 16px; text-align: center; border-bottom: 1px solid var(--theme-neutral-200);">Days</th>
            ...
            <tbody id="leaveRequestsTableBody">
                ...
                <tr data-department="{{ $application->employee->employmentDetail->departmentRelation->name ?? 'N/A' }}"
                    ...
                    <td style="padding: 14px 16px; border-bottom: 1px solid var(--theme-neutral-200);">
                        <div class="emp-cell" style="display: flex; align-items: center; gap: 12px;">
                            @if($application->employee->photo)
                                <img src="{{ $application->employee->photo }}" alt="{{ $application->employee->first_name }}" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2px solid var(--theme-neutral-300); flex-shrink: 0;">
                            @else
                                <div style="background: {{ $avatarColors[($application->employee->id ?? 0) % count($avatarColors)] }}; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 700; font-size: 13px; border: 2px solid var(--theme-neutral-300); flex-shrink: 0;">
                                    {{ strtoupper(substr($application->employee->first_name ?? 'N', 0, 1) . substr($application->employee->last_name ?? 'A', 0, 1)) }}
                                </div>
                            @endif
                            <div style="min-width: 0;">
                                <p style="margin: 0 0 2px; font-size: 13px; font-weight: 600; color: var(--theme-neutral-900); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $application->employee->first_name ?? 'N/A' }} {{ $application->employee->last_name ?? '' }}</p>
                                <p style="margin: 0; font-size: 11px; color: var(--theme-neutral-700); font-weight: 500;">{{ $application->employee->employee_id ?? 'N/A' }}</p>
                            </div>
                        </div>
                    </td>
                    <td style="padding: 14px 16px; border-bottom: 1px solid var(--theme-neutral-200);">
                        <span style="display: inline-block; padding: 4px 10px; border-radius: 999px; font-size: 11px; font-weight: 700; background: var(--theme-neutral-100); color: var(--theme-neutral-700);">{{ $application->employee->employmentDetail->departmentRelation->name ?? 'N/A' }}</span>
                    </td>
                    <td style="padding: 14px 16px; border-bottom: 1px solid var(--theme-neutral-200); font-size: 13px; color: var(--gp-pri); font-weight: 600;">{{ $application->leaveType->leave_name ?? 'N/A' }}</td>
                    <td style="padding: 14px 16px; border-bottom: 1px solid var(--theme-neutral-200); font-size: 12px; color: var(--theme-neutral-700);">
                        @php
                        ...
                        @endphp
                        <span style="font-weight: 600; color: var(--theme-neutral-950);">{{ $period }}</span>
                    </td>
                    <td style="padding: 14px 16px; border-bottom: 1px solid var(--theme-neutral-200); text-align: center; font-weight: 700; color: var(--gp-pri); font-size: 14px;">{{ number_format($application->number_of_days, 1) }}</td>
                    ...
                    <td class="row-menu-cell" style="padding: 14px 16px; border-bottom: 1px solid var(--theme-neutral-200);">
                        <div style="position: relative; display: flex; justify-content: center;">
                            ...
                            <div class="leave-action-menu" style="display: none; position: absolute; right: 0; top: 100%; background: #fff; border: 1px solid var(--theme-neutral-300); border-radius: 10px; box-shadow: 0 10px 24px rgba(15, 23, 42, 0.15); z-index: 100; min-width: 160px; margin-top: 6px; overflow: hidden;">
                                ...
                                @if($application->status === 'pending')
                                    <button onclick="approveLeaveRequest({{ $application->id }}, '{{ $application->application_number }}')" style="width: 100%; padding: 11px 14px; border: none; background: none; text-align: left; font-size: 12px; color: #22c55e; font-weight: 600; cursor: pointer; border-top: 1px solid var(--theme-neutral-100); transition: all 0.2s; border-radius: 0;" onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background='none'">
                                    ...
                                    <button onclick="openRejectModal({{ $application->id }}, '{{ $application->application_number }}')" style="width: 100%; padding: 11px 14px; border: none; background: none; text-align: left; font-size: 12px; color: #ef4444; font-weight: 600; cursor: pointer; border-top: 1px solid var(--theme-neutral-100); transition: all 0.2s; border-radius: 0;" onmouseover="this.style.background='#fef2f2'" onmouseout="this.style.background='none'">
                ...
                <tr>
                    <td colspan="7" style="text-align: center; padding: 60px 20px; border-bottom: none;">
                        ...
                        <p style="margin: 0 0 8px; font-size: 15px; color: var(--theme-neutral-700); font-weight: 600;">No leave requests found</p>
                        <p style="margin: 0; font-size: 13px; color: var(--theme-neutral-600);">Leave applications will appear here</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="table-footer" style="display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border-top: 1px solid var(--theme-neutral-200); background: var(--theme-neutral-50);">
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <select id="leaveRequestRowsPerPage" onchange="changeLeaveRequestRowsPerPage()" style="width: auto; padding: 6px 10px; font-size: 12px; border: 1px solid var(--theme-neutral-300); border-radius: 6px; background: #fff; color: var(--gp-text-mid); font-weight: 600; cursor: pointer;">
    ...
    <div class="modal-box" onclick="event.stopPropagation()" style="max-width: 500px;">
        ...
        <div class="modal-body">
            <div class="form-field">
                ...
                <textarea id="rejectionReason" rows="4" placeholder="Explain why this leave request is being disapproved..." required style="width: 100%; padding: 12px; border: 2px solid var(--theme-neutral-300); border-radius: 8px; font-family: inherit; font-size: 13px; resize: vertical;"></textarea>
            </div>
        </div>
        <div class="modal-footer">
            <button class="modal-btn-ghost" onclick="closeRejectModal()">Cancel</button>
            <button class="modal-btn-primary" id="confirmRejectBtn" style="background: var(--theme-danger);">
