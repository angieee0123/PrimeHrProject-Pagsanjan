    ...
    <section class="table-section report-filter-bar">
        <form method="GET" action="{{ route('admin.reports') }}" class="table-header" id="reportFilterForm">
            ...
            <div class="table-actions">
                ...
                {{-- Exports the tab that is open, not "the reports page": each
                     tab reports a different thing, so the endpoint switches
                     with the tab rather than handing out one merged file. The
                     period below is sent as query params and the endpoint
                     re-runs the report server-side — it never reads the table
                     on screen, which carries percentage bars and status chips
                     rather than figures. --}}
                <button type="button" class="btn-export adm-btn-primary-solid" id="reportExportBtn" onclick="exportReport()">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    Export CSV
                </button>
                <button type="button" class="btn-export" onclick="window.print()" title="Print the report on screen">
...
<script>
    ...
    // One export endpoint per tab. Keyed by tab id so switchReport() does not
    // have to know any of them — it only sets #reportTabInput, which is what
    // this reads back.
    window.reportExportUrls = @json(collect($reportTypes)->mapWithKeys(fn ($rt, $id) => [$id => route('admin.reports.export.' . $id)]));

    /**
     * Download the open tab's report as CSV.
     *
     * The period is read out of the filter form rather than off the URL, so a
     * period changed but not yet applied still exports what the selects say —
     * the same rule `exportPayrollRegister` and `exportAttendanceSummary`
     * follow. The endpoint recomputes the report from those values; nothing
     * here scrapes the rendered table.
     */
    function exportReport() {
        const tab = document.getElementById('reportTabInput').value;
        const url = window.reportExportUrls[tab];
        if (!url) return;

        const form = document.getElementById('reportFilterForm');
        const params = new URLSearchParams({ tab });

        ['semi', 'month', 'year'].forEach(name => {
